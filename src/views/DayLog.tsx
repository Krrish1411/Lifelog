...
export function DayLogView() {
  ...
  }, [state.tasks, sel, state.settings.showLifeLogProject]);
  ...
  const sleepMin = useMemo(() => {
    if (state.settings.showLifeLogProject === false) return 0;
    return state.tasks
  ...
  const generateStandupMarkdown = () => {
    ...
    const sleepSection = showLifeLog
      ? `\n#### 😴 Sleep & Rest\n${sleepSummary}\n`
      : "";
    const lifeHabitSection = showLifeLog
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="card engine-panel grid grid-cols-7 gap-1.5 p-2 w-full min-w-0 overflow-hidden">
        {week.map((iso, i) => {
          ...
          const hasLife = showLifeLog && state.tasks.some((t) => t.projectId === LIFE_LOG_PROJECT_ID && (t.due === iso || (t.done && t.doneAt && isoDate(new Date(t.doneAt)) === iso)));
          const dayDoneCount = state.tasks.filter((t) => (showLifeLog || t.projectId !== LIFE_LOG_PROJECT_ID) && ((t.done && t.doneAt && isoDate(new Date(t.doneAt)) === iso) || t.completions.some((c) => isoDate(new Date(c.at)) === iso))).length;
          return (
            <button
              key={iso}
              onClick={() => pickDay(iso)}
      ...
      {/* 24-Hour Whole Day Balance & Focus Log Bar (Only when LifeLog is enabled) */}
      {showLifeLog && (
      ...
      <div
        className={cn(
          "stagger grid gap-2.5 w-full min-w-0",
          showLifeLog ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-2 lg:grid-cols-3"
        )}
      >
        {[
            ...
            : [
                {
                  icon: <Flame size={16} />,
                  k: "Habits Kept",
                  v: String(state.habits.filter((h) => h.completions.includes(sel)).length),
                  sub: `${state.habits.length} habits`,
                  hot: state.habits.some((h) => h.completions.includes(sel)),
                },
              ]),
          {
            icon: <CheckCircle2 size={16} />,
            k: "Tasks Done",
            v: String(dayDone.length),
            sub: showLifeLog
              ? `${dayLifeLogTasks.filter((t) => t.done).length} routines`
              : `${dayDone.length} completed`,
            hot: dayDone.length > 0,
          },
        ].map((x) => (
      ...
      <div
        className={cn(
          ...
          state.settings.showLifeLogProject !== false ? "lg:grid-cols-[1.15fr_1fr]" : "grid-cols-1"
        )}
      >
        ...
        <div className="card engine-panel p-4 w-full min-w-0 overflow-hidden">
          <div className="flex items-center justify-between">
            <div className="font-display text-[15px] font-bold tracking-tight">
              {showLifeLog ? "Tasks & Activities Completed" : "Tasks Completed"}
            </div>
        ...
        {state.settings.showLifeLogProject !== false && (
          <div className="card engine-panel p-4 w-full min-w-0 overflow-hidden">
            <div className="flex items-center justify-between">
